...
declare_id!("J9UWzu1hScBJkw4MLqrfQvKMjyFj3jAKJWGASYmEnvFn");
...
pub mod signatures {
    ...
    pub fn verify_ed25519(ctx: Context<Verify>, bumpy: u8, swap_option: u8, amount: u64, tx_id: String, pubkey: [u8; 32], msg: Vec<u8>, sig: [u8; 64]) -> Result<()> {
...
pub mod utils {
    ...
    pub fn verify_ed25519_ix(ix: &Instruction, pubkey: &[u8], msg: &[u8], sig: &[u8]) -> Result<()> {
        ...
        {
            return Err(VerifyError::SigVerificationFailed.into());    // Otherwise, we can already throw err
        }
    ...
    pub fn check_ed25519_data(data: &[u8], pubkey: &[u8], msg: &[u8], sig: &[u8]) -> Result<()> {
        ...
        {
            return Err(VerifyError::SigVerificationFailed.into());
        }
